...
__attribute__((__export_name__("[callback][async-lift]wasi:cli/run@0.3.0-rc-2025-09-16#run")))
uint32_t __wasm_export_exports_wasi_cli_run_run_callback(uint32_t event_raw, uint32_t waitable, uint32_t code) {
  ...
  event.event = (wasip3_event_code_t) event_raw;
  event.waitable = waitable;
  event.code = code;
  return exports_wasi_cli_run_run_callback(&event);
}

__attribute__((__import_module__("[export]wasi:cli/run@0.3.0-rc-2025-09-16"), __import_name__("[task-return]run")))
...
__attribute__((__export_name__("[async-lift]wasi:cli/run@0.3.0-rc-2025-09-16#run")))
int32_t __wasm_export_exports_wasi_cli_run_run(void) {
  wasip3_callback_code_t ret = exports_wasi_cli_run_run();
  return ret;
}
